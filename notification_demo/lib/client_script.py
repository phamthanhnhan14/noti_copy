...
def create_client_script(event):
...
def add_renderer_globals(event):
    event['ClientScript'] = sys.modules[__name__]
...
def includeme(config):
    config.add_subscriber(create_client_script, NewRequest)
    config.add_subscriber(add_renderer_globals, BeforeRender)
    config.add_tween('notification_demo.lib.client_script.client_script_tween_factory')
